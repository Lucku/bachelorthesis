...
int main()
{
	...
	b->registerFunction(vByteEncodeEncrypted, DIVMULTADD(4, 5, AES_BLOCK_SIZE), "compression_enc", 16);
	b->registerFunctionWithPreprocessing(vByteDecodeEncrypted, MULT(4), vByteEncodeEncryptedPreproc, MULTADD(5, AES_BLOCK_SIZE), "decompression_enc", 16);
	...
	b->registerFunctionWithPreprocessing(completeProcess, MULTADD(10, AES_BLOCK_SIZE), vByteEncodeEncryptedPreproc, MULTADD(5, AES_BLOCK_SIZE), "complete", 16);
	b->registerFunction(crypto, MULTADD(1, AES_BLOCK_SIZE), "crypto", 16);
