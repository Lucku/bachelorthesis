...
static sgx_status_t SGX_CDECL sgx_enclaveVByteEncodeEncrypted(void* pms)
{
	...
	enclaveVByteEncodeEncrypted(_in_in, _tmp_inLength, _in_out, _tmp_outLength);
err:
	if (_in_in) free(_in_in);
	if (_in_out) {
		memcpy(_tmp_out, _in_out, _len_out);
...
static sgx_status_t SGX_CDECL sgx_enclaveVByteDecodeEncrypted(void* pms)
{
	...
	enclaveVByteDecodeEncrypted(_in_in, _tmp_inLength, _in_out, _tmp_outLength);
err:
	if (_in_in) free(_in_in);
	if (_in_out) {
		memcpy(_tmp_out, _in_out, _len_out);
...
static sgx_status_t SGX_CDECL sgx_encryptBytes(void* pms)
{
	...
	size_t _tmp_outLength = ms->ms_outLength;
	size_t _len_output = _tmp_outLength;
	...
	if (_tmp_output != NULL) {
		_in_output = (uint8_t*)malloc(_len_output);
		if (_in_output == NULL) {
			status = SGX_ERROR_OUT_OF_MEMORY;
			goto err;
		}

		memcpy(_in_output, _tmp_output, _len_output);
	}
	if (_tmp_key != NULL) {
	...
	ms->ms_retval = encryptBytes(_in_input, _tmp_inLength, _in_output, _tmp_outLength, (const uint8_t*)_in_key, _tmp_keyLength, _in_iv);
err:
...
static sgx_status_t SGX_CDECL sgx_decryptBytes(void* pms)
{
	...
	ms->ms_retval = decryptBytes(_in_input, _tmp_inLength, _in_output, _tmp_outLength, (const uint8_t*)_in_key, _tmp_keyLength, _in_iv);
err:
	if (_in_input) free(_in_input);
	if (_in_output) free(_in_output);
	if (_in_key) free((void*)_in_key);
